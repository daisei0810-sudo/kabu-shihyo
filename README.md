...
OSの環境変数として設定する他、リポジトリルートに`.env`ファイル（`.env.example`を
コピーして作成、gitignore対象）を置けば`python -m src.main`起動時に自動読み込みされる
（既存のOS環境変数やGitHub Actions secretsは上書きしない）。
...
**GitHub Actions（daily.yml）で自動実行する場合は、上記キーをリポジトリのSecretsにも
別途登録する必要がある**（ローカルの`.env`/環境変数とは独立。`gh secret set FRED_API_KEY`等、
または GitHub の Settings → Secrets and variables → Actions から設定）。
...
# Step5: 材料取込（SEC EDGAR + EDINET + RSS + 手動入力。--step all に含む）
...
**マクロ (FRED)**  
→ `FRED_API_KEY` をセットすると米国製造業景況感指数(OECD)・鉱工業生産・耐久財受注・
FFレート等を取得

**ISM製造業PMI(本家、手動更新)**  
FRED/DBnomicsいずれも無料での自動取得が2025年後半以降不可能(ISM社のライセンス制限、
詳細: docs/investment_os_design.md)。`config/ism_pmi_manual.csv`にISM公式プレスリリース
(https://www.ismworld.org/)の月次数値を手入力する運用。2020-05〜2025-08分は検証済み
実データを事前投入済み。更新が滞ると`daily_report.md`の「手動更新指標の鮮度」節に
自動で警告が表示される(忘れ防止)
...
- [x] **Step5**: 材料取込基盤（SEC EDGAR全文検索 + EDINET + RSS + 手動入力 → material_id・重複検知・
  鮮度スコア・ソースランク → `data/materials/*.jsonl`）。EDINET動作確認済み、EDGARは全文検索の
  性質上ノイズ(無関係企業のヒット)が混じるが`company_hint`優先ロジックで実害を抑制。
  `related_tickers`(材料→保有銘柄の紐付け)は`material_id.py`の`resolve_related_ticker()`が
  config.INSTRUMENTS由来のトークンのみ自動採用し、追跡対象外企業は紐付けない(捏造回避)。
  `--step all`に含む（実行順は`1→2→3→5→8→10→12→7→6→4`、policy_tailwind軸・
  regulation/dilution/customer_churnリスクより前に実行）
