...
src/
  ...
  prediction/        # Step7: 予測台帳・事後評価(Investment OS Layer5、最重要レイヤー)
...
data/predictions/    # 予測台帳正本 (JSONL。3/6/12ヶ月後の答え合わせをここに蓄積)
...
outputs/             # レポート・CSV・グラフ(公開、GitHub Pagesへデプロイ)
private/             # 投資判断・配分等の非公開出力(gitignore対象、§8確定事項)
tests/               # pytest テスト
...
- [x] **Step9**: 意思決定エンジン（Investment OS Layer2）— `config/scenarios/*.yaml` の
  ...
  出力は保有情報を含むため非公開（`private/decision_report.md`、`--step all` には未含有）
