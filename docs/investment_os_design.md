...
| L10非公開レポート更新 | ✅完了 | `decision_report.py`のリスク(L6)・配分(L9)セクションを「未実装」から実データ表示へ差し替え。発掘(L7-8)はP4で実装済み(§11参照) |
| L10レポート再構成 | ✅完了(P3後半、ユーザー指摘対応) | 従来はテーマスコア/シグナル/テクニカル/押し目売り/投資判断が別々の章に分散し、1銘柄の判断根拠を理解するのに複数表を往復する必要があった。`_section_holdings_detail()`で保有銘柄1件=1章に全根拠(テーマスコア内訳・シグナル・テクニカル・リスク・成立条件の実際の文言)を集約する構成に刷新。冒頭に📋サマリー章を新設 |
...
## 11. P4実装状況(2026-07-07)

P4スコープ(L7新規投資発掘・L8新テーマ発掘)を実装済み。両レイヤーとも保有銘柄の
判断を一切含まないため、他レイヤーと異なり公開データ(`outputs/`)として扱う。

| 項目 | 状態 | 備考 |
|---|---|---|
| L7 `src/discovery/companies.py` | ✅完了(`--step 12`、`all`に含む) | ユニバースは`instruments.csv`の`held=False`銘柄(13件)。`expected_value`は個社期待値モデルが未整備のため現状theme_scoreをそのまま採用(個社ファンダメンタルズ差の指標ではなく「所属テーマの地合い」の代理指標であることを明示、捏造回避)。差別化要素として「テーマ内相対モメンタム」(自分の直近65営業日騰落率 − 同テーマ全銘柄平均)を別カラムで併記。テーマ内比較の母集団は保有/非保有を問わない(業界内の相対的強さを見たいだけなので保有状況とは無関係)。philosophy.yaml(ボトルネック性の手動タグ)は未整備のため設計書の(3)は見送り、(1)(2)(4)のみ実装 |
| L8 `src/discovery/themes.py` | ✅完了(`--step 12`、`all`に含む) | ユニバースは`themes.csv`の`status=watch`テーマ(現状バイオのみ)。未登録テーマの自動発掘(クラスタリング等でゼロから新テーマを発見)は、現状のmaterials蓄積量(2026-07時点で6件、related_tickers紐付けも未整備)では統計的意味を持たないため実装しない。新テーマを監視対象にしたい場合は`themes.csv`へ`status=watch`で手動登録すれば本モジュールが自動追跡し始める設計。tam_estimate/growth_rate/feasibilityは無料データソースがないため常に「未整備」と正直に返す(捏造回避)。唯一の定量シグナルはmaterialsのキーワード出現件数トレンド(`WATCH_THEME_KEYWORDS`で手動管理、bioのみ定義済み) |
| L10非公開レポート統合 | ✅完了 | `decision_report.py`の`_section_discovery()`が`outputs/discovery_companies.csv`・`outputs/discovery_themes.csv`を読み込み実データ表示(従来の「未実装」プレースホルダーを置換) |
| `main.py`配線 | ✅完了 | Step12として追加、`--step all`に含む(公開データのため9・11と異なりPRIVATE_REPO_PAT非依存)。実行順は`1→2→3→8→10→12→7→6→4` |
...
### config/rss_sources.csv への初の実在URL登録(2026-07-08追記)

WebSearchで企業別・省庁別RSSフィードの候補URLを調査したが、推測パターンでは
特定できなかった(METI関連ドメイン3パターンは全て403 Forbidden、PR TIMES企業別
フィードも4パターン試して全て404)。**ユーザーが実際にブラウザでフジクラのPR TIMES
企業ページ(`https://prtimes.jp/main/html/searchrlp/company_id/56990`)を開き、
「RSSを購読する」リンクの実URLを確認**したことで解決(`https://prtimes.jp/
companyrdf.php?company_id=56990`)。推測ベースの探索には限界があり、実URLの
特定には人間による目視確認が最も確実という教訓。

`config/rss_sources.csv`にこのURLを登録し`--step 5`で検証した結果:
- materials.db: 9件 → **209件**に急増(203件でrelated_tickers正しく紐付き)
- `ai_datacenter`テーマの政策追い風軸が`unavailable` → **40.0**に変化
  (該当軸に実際にmaterialsが反映された初の実例)、confidence(データ被覆率)も55%→60%

**判明した既知の脆弱性(未修正)**: 初回バックフィル時、同日・同トピック(`MISC`)の
材料が`generate_material_id`の衝突サフィックス上限(`_2`〜`_99`)に達し、
`RuntimeError`で該当バッチのRSS取込処理全体が打ち切られた(`run_ingest`の
RSS処理ブロックが単一の`try/except`で全ソース分をまとめて捕捉しているため)。
初回の大量バックフィル特有の現象で日次運用(1日あたり数件程度)では再発しにくいが、
複数のRSSソースを登録した場合、1ソースの衝突で他ソース分の処理も打ち切られる
潜在的なリスクが残る。対応は見送り、既知の課題として記録するに留めた。
