...
保存方式は score_history.py と同じ思想(outputs/history/配下に日次追記・git管理・
履歴不足時はNoneを返し捏造しない)を踏襲するが、1銘柄1CSVではなく「全銘柄を1日1CSV」の
スナップショット方式にする(銘柄追加時のファイル増殖回避・比較の一括読込のため)。
"""
...
DECISIONS_HISTORY_DIR = Path(OUTPUTS) / "history" / "decisions"
