"""Plotly ダッシュボード生成 + PWA アセット出力(公開、GitHub Pagesへデプロイ)。
...
保有銘柄ごとのスコア・outlook・action・テクニカル判定・押し目売り時判定は売買判断
そのもの(docs/investment_os_design.md §8確定事項)のため、本モジュールは扱わない。
公開Pagesに載るのは集計指標(サイクルスコア・実需指数・XRP集計スコア・マクロ・
指標スコアカード)のみ。保有銘柄の詳細は private/decision_report.md (非公開)を参照。
"""
...
def _make_xrp_gauges(xrp_df: pd.DataFrame) -> str:
    """XRPロック需要 + 総合実需ゲージ → plotly div 文字列(公開、集計スコアのみ)。"""
    if xrp_df.empty or "target" not in xrp_df.columns:
        return "<p style='color:#888'>XRPスコアデータなし (Step3 未実行)</p>"
    ld_row = xrp_df[xrp_df["target"] == "xrp_lock_demand"]
    rd_row = xrp_df[xrp_df["target"] == "xrp_real_demand"]
...
<div class="warn">
  ℹ️ 保有銘柄ごとの投資判断(outlook/action・テクニカル判定・押し目売り時判定)は
  非公開レポート(private/decision_report.md)に統合されています。
</div>
...
def build_dashboard() -> None:
    ...
    xrp_div       = _make_xrp_gauges(xrp_df)
