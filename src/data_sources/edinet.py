...
【重要な注意】このモジュールはEDINET API v2の公開ドキュメントに基づき実装しているが、
実際のAPIキーでの動作検証ができていない(本セッションではキーを取得していないため)。
エンドポイントURL・パラメータ名・レスポンス構造に誤りがある可能性があるため、
実際に使用する前に必ず `python -m src.main --step 5` で取得0件が続いていないか、
ログにエラーが出ていないかを確認すること。失敗時は既存 data_sources/ 全体の方針通り
クラッシュせず空リストを返す。
"""
